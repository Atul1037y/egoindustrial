...
class InferenceRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "video": [[[[0.5 for _ in range(224)] for _ in range(224)] for _ in range(16)] for _ in range(3)]
            }
        }
    )
...
def get_top5_predictions(probs: list[float], labels: list[str]) -> list[dict[str, Any]]:
    ...
    return [
        {"label": labels[i] if i < len(labels) else f"class_{i}", "probability": float(probs[i]), "class_id": int(i)}
        for i in top5_idx
    ]
